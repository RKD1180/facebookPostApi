...
const Home = () => {
  ...
  const onSubmit = data => {
    ...
    if (!url) {
      ...
      fetch(`https://graph.facebook.com/v13.0/${process.env.REACT_APP_PAGE_ID}/feed?published=true&message=${data?.text}&access_token=${process.env.REACT_APP_ACCESS_TOKEN}`, {
        method: "post"
      })
        .then(res => res.json())
        .then(data => {
    ...
    if (url) {
      fetch(`https://graph.facebook.com/v13.0/${process.env.REACT_APP_PAGE_ID}/photos?url=${data?.img}&published=true&message=${data?.text}&access_token=${process.env.REACT_APP_ACCESS_TOKEN}`, {
        method: "post"
      })
        .then(res => res.json())
        .then(data => {
